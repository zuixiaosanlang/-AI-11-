...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    # build the model
    shadow_free_image = build_aggasatt_joint2(input, channel, vgg_19_path=vgg_19_path)

    # Perceptual Loss
    loss_percep = compute_percep_loss_2(shadow_free_image, target, vgg_19_path=vgg_19_path)
...
if is_training:
    ...
    JSON = "Dataset/inpainting/annos.txt"
    ...
    for epoch in range(1, maxepoch):
        ...
        for id in np.random.permutation(num_train):
            ...
            if input_images_ids[id] is None:
                ...
                neww = np.random.randint(400, 450)
                newh = np.random.randint(350, 400)
                ...
                if magic < ARGS.use_da:  # choose from fake images
                    ...
                    # iminput, imtarget = parpare_image2(current_img_id, (neww, newh), da=True,
                    #                                           stage=['_M', '_C', '_B'])
                    iminput, imtarget = parpare_image3(current_img_id, json_data, (neww, newh), da=True)
                ...
                if cnt % 2 == 0:
                    fetch_list = [d_opt]
                    # update D
                    _ = sess.run(fetch_list, feed_dict={input: iminput, target: imtarget})

                # update G
                fetch_list = [g_opt, shadow_free_image, d_loss, g_loss, loss, loss_percep]
                _, imoutput, current_d, current_g, current, current_percep = \
                    sess.run(fetch_list, feed_dict={input: iminput, target: imtarget})
